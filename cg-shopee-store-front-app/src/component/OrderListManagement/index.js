...
import DialogConfirmOrder from "../../component/DialogConfirmOrder";
import {GHTK_CHECK_ORDER} from "../../constant/link";
import {Link} from "react-router-dom";


const OrderListManagement = ({orderList, isLoading, onPostOrderSuccess}) => {
    ...
    const rowExpansionTemplate = (rowData) => {
        return (
            <div className="p-3">
                <h5>Order Detail</h5>
                <DataTable value={rowData.orderDetails} dataKey="id">
    ...
    const buyerBodyTemplate = (rowData) => {
        return (
            <div className="flex align-items-center gap-2">
                <Avatar src={rowData.buyer.avatar} size={30}/>
                <span className="ms-2">{rowData.buyer.username}</span>
            </div>
        );
    };
    ...
    const handleConfirmOrder = (rowData) => {
        setOpenDialogConfirmOrder(true);
        setOrderToConfirm(rowData);
    }

    const processBodyTemplate = (rowData) => {
        return rowData.status === "REQUESTING"
            ? <Button label="Prepare goods"
                      icon="pi pi-check"
                      size="small"
                      severity="warning"
                      onClick={() =>handleConfirmOrder(rowData)}
            />
            : rowData.deliveryTrackingCode
    ...
    const handleUpdateOrderAfterPost = (order) => {
        onPostOrderSuccess(order)
    }
    ...
    return (
        <div className="card">
            ...
            <DialogConfirmOrder isOpen={openDialogConfirmOrder}
                                order={orderToConfirm}
                                onHide={() => setOpenDialogConfirmOrder(false)}
                                onPostOrderSuccess={handleUpdateOrderAfterPost}
            />
        </div>
    );
};
